    ...
    double integrate(const std::function<double(double)>& func) {
        ...
        if (rank == 0) {  // Master process
            ...
            for (int i = 1; i < size; ++i) {
                size_t slave_points = points_per_slave + (i <= remaining_points ? 1 : 0);
                MPI_Send(&slave_points, 1, MPI_UNSIGNED_LONG, i, 0, MPI_COMM_WORLD);
            }
            
            for (int i = 1; i < size; ++i) {
                double slave_result;
                MPI_Recv(&slave_result, 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                global_sum += slave_result;  // 1 FLOP (addition)
            }
            
            // Final calculation
            global_sum = (b - a) * (global_sum / nPoints);  // 3 FLOPs (subtraction, multiplication, division)
            flopCount = (size - 1) + 3;  // Count master's FLOPs
        } 
        else
        {  // Slave processes
          size_t local_points;
          MPI_Recv(&local_points, 1, MPI_UNSIGNED_LONG, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      ...
      size_t global_flops;
      MPI_Reduce(&flopCount, &global_flops, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
...
int main(int argc, char *argv[]) {
    ...
    monteCarloIntegrator integrator(lower_bound, upper_bound, num_points, rd,MPI_COMM_WORLD);
